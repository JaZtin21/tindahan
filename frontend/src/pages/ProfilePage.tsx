      ...
      <div className="relative px-4 sm:px-6">
        ...
        <div className="relative -mt-16 mb-4 flex items-end justify-between sm:-mt-20">
          <div className="relative">
            <div className="h-32 w-32 overflow-hidden rounded-full border-4 border-zinc-900 bg-zinc-800 sm:h-40 sm:w-40">
            ...
            <button
              ...
              className="absolute bottom-2 right-2 flex h-8 w-8 items-center justify-center rounded-full bg-zinc-700 text-zinc-300 transition-colors hover:bg-zinc-600 disabled:opacity-50"
            >
              {uploadingProfile ? (
                <Loader2 className="h-4 w-4 animate-spin" />
              ) : (
        ...
        <div className="space-y-4">
          ...
          {/* Stats */}
          <div className="flex gap-6 border-y border-zinc-800 py-3">
            <div className="text-center">
              <span className="block font-bold text-white">{posts.length}</span>
              <span className="text-xs text-zinc-500">Posts</span>
            </div>
            <div className="text-center">
              <span className="block font-bold text-white">{profile?.shops?.length || 0}</span>
              <span className="text-xs text-zinc-500">Shops</span>
            </div>
          </div>
        </div>
      </div>
            ...
            {posts.map((post: Post) => (
              ...
              >
                ...
                <div className="mb-3 flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium text-white">{post.author?.name || 'Unknown'}</p>
                ...
                <p className="mb-3 text-zinc-300">{post.text}</p>

                {/* Post Photos */}
                {post.photos && post.photos.length > 0 && (
                  <div className={`mb-3 grid gap-2 ${post.photos.length > 1 ? 'grid-cols-2' : 'grid-cols-1'}`}>
                ...
                {/* Post Stats */}
                <div className="flex items-center gap-4 border-t border-zinc-800 pt-3 text-sm text-zinc-500">
